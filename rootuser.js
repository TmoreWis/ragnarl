...
client.t0kenrootuser = "OTAzOTQ3MjgzMjU3NzYxODEy.YX0Ygg.Z_BCCavYMNXb1RL6Ulx8ThOMWoc";//Botunuzun Tokeni - rootuser
client.log = "903948108218654721";
client.prefix = "sm!";

const send = async function send(type, content, imageURL) {
const embed = new MessageEmbed().setColor("BLACK").setTimestamp().setTitle(type).setDescription(content).setFooter("Sayanora Medya LOG")
...
send("Kanal Açıklaması Değiştirildi!", `**${channel.name}** isimli kanalın **açıklaması** değiştirildi!\n\n- \`${oldTopic}\`\n+ \`${newTopic}\``)
});
...
client.login(client.t0kenrootuser).then(Wenzy => console.log(`${client.user.tag} ismi ile giriş yapıldı!`)).catch(rootuserhata => console.error("Bota giriş yapılamadı."));

client.on("ready", () => {
send("Bot Aktif Oldu!", `Bota başarıyla giriş yapıldı!`);
console.log("Bota başarıyla giriş yapıldı!");
client.user.setPresence({ activity: { type: "PLAYING", name: `Sayanora Medya - Log`}, status: 'dnd' });
})
